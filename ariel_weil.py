...
def get_stdev(nums):
    ...
    for num in nums:
        sq_err=(avg-num)**2
        cum_err=cum_err+sq_err
    return((cum_err/(len(nums)-1))**0.5)
...
def get_standard_error(a, b):
    ...
    '''
    a_res=(get_stdev(a)**2)/len(a)
    b_res=(get_stdev(b)**2)/len(b)
    return (a_res+b_res)**0.5
...
def get_2_sample_df(a, b):
    ...
    '''
    a_res=(((get_stdev(a)**2)/len(a))**2)/(len(a)-1)
    b_res=(((get_stdev(b)**2)/len(b))**2)/(len(b)-1)

    return round((get_standard_error(a,b)**4)/(a_res+b_res))
...
def get_t_score(a, b):
    ...
    '''
    #print("a=" + str(get_avg(a))+ ", " + str(get_stdev(a)))
    #print("b=" + str(get_avg(b))+ ", " + str(get_stdev(b)))
...
def get_expected_grid(observed_grid):
    ...
    ** DO NOT modify the parameter, observed_grid. **
    ...
    row_totals=[0 for x in range(rows)]
    col_totals=[0 for x in range(cols)]
    for i in range(rows):
        for j in range(cols):
            row_totals[i]=row_totals[i]+observed_grid[i][j]
            col_totals[j]=col_totals[j]+observed_grid[i][j]

    sum=0
    for num in row_totals:
        sum=sum+num
    ...
    for i in range(rows):
        for j in range(cols):
            res[i][j]=row_totals[i]*col_totals[j]/sum
...
print("chi2 test for returns")
y_list= data_to_num_list(y_returns)
n_list = data_to_num_list(n_returns)
observed_grid = [y_list, n_list]
